...
func (c *callContext) Memory() wasm.Memory {
	return &memory{wazero: c.wazero.Memory()}
}

func (c *callContext) ExportedFunction(name string) wasm.Function {
	return &importedFn{wazero: c.wazero.ExportedFunction(name)}
}

func (c *callContext) ExportedMemory(name string) wasm.Memory {
	return &memory{wazero: c.wazero.ExportedMemory(name)}
}

func (c *callContext) ExportedGlobal(name string) wasm.Global {
...
func (f *importedFn) Definition() wasm.FunctionDefinition {
...
func (f *importedFnDef) ParamTypes() []wasm.ValueType {
	_vt := f.wazero.ParamTypes()
	count := len(_vt)
	vt := make([]wasm.ValueType, count)
	for i := 0; i < count; i++ {
		vt[i] = wasm.ValueType(_vt[i])
	}
	return vt
}

func (f *importedFnDef) ResultTypes() []wasm.ValueType {
...
func (g *global) Type() wasm.ValueType {
	return wasm.ValueType(g.wazero.Type())
}
